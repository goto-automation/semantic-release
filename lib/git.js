 ...
 * @throws {Error} If the `git` command fails.
 */
export async function getNoMergeTags(branch, execaOptions) {
 ...
 */
export async function getGitRemoteHead(repositoryUrl, branch, execaOptions) {
  return (await execa("git", ["ls-remote", "--heads", repositoryUrl, branch], execaOptions)).stdout
    .split("\n")
    .map((branch) => branch.match(/^(?<hash>\w+)\s*refs\/heads\/(?<branch>.+)$/))
    .filter(Boolean)
    .reduce((result, match) => {
      result[match.groups.branch] = match.groups.hash;
      return result;
    }, {})[branch];
}
 ...
 */
export async function isAncestor(first, second, execaOptions) {
  try {
